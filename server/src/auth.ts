...
import { db, getUserById, type UserRow } from './db.js'
...
authRouter.post('/signup', (req: Request, res: Response) => {
  ...
  const name = typeof displayName === 'string' && displayName.trim() ? displayName.trim() : username
  if (name.length > 20) {
...
meRouter.patch('/', (req: Request, res: Response) => {
  ...
  const { displayName, trackingEnabled, shareCycle, cycleLenOverride, periodLenOverride, onboarded } =
    req.body ?? {}
  ...
  if (keys.length > 0) {
    ...
    db.prepare(`UPDATE users SET ${setSql} WHERE id = ?`).run(...keys.map((k) => updates[k]), me.id)
  }
  res.json({ user: publicUser(getUserById(me.id)!) })
})
